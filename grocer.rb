def consolidate_cart(item)
  final = Hash.new 0 #zero sets default to zero and not nill 
  ...
  if final.has_key?(food) == false
    ...
    final[food][count] = 1 # if the food never was counted before, set initial value to 1. 
  else if final.has_key?(food) # if it does exist we are incrementing count by 1. 
  final[food][:count] +=1
